...
git_dirty="$(test -n "$(git status --porcelain --ignore-submodules=dirty)" && echo true || echo false)"
if [[ "${git_head}" != "${expected_git_sha}" || "${git_dirty}" != "false" ]]; then
  echo "formal acceptance requires clean ${expected_git_sha}; got head=${git_head} dirty=${git_dirty}" >&2
  exit 2
fi
{
...
printf "experiment_kind\tmode\tseed\tarm\tsubject_arm\tarm_order\texit_code\trun_dir\tplaybook_before_sha256\tplaybook_after_sha256\tplaybook_before_snapshot\tplaybook_after_snapshot\tgit_head\tgit_dirty\n" \
  > "${status_file}"
overall_status=0
...
run_arm() {
  ...
  printf "behavior\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
...
run_shadow_calibration() {
  ...
  printf "calibration\t%s\t%s\tshadow\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
